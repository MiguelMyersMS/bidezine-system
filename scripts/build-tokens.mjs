...
// config's catch-all colour / shadow-colour groups stop swallowing them (Issue 07l).
// Derived from the same typography and shadow composites the CSS is built from — see the
// emitter's own comment. Not committed; regenerated by the pre* hooks alongside tokens.css.
...
console.log(
  `tw-merge-groups: ${fontSizeRoles.length} font-size role(s), ${shadowElevationNames.length} elevation name(s) → src/tw-merge-groups.js`
)
