...
// ── tailwind-merge class-group names (Issue 07l) ───────────────────────────────────
// cn() (src/lib/utils.ts) runs tailwind-merge, whose DEFAULT config knows none of our
// custom named utilities. Its catch-all colour validator (theme `color`, declared
// [isAny]) claims every `text-<role>` as a COLOUR, so a role and a colour in one class
// string land in the same conflict group and tailwind-merge deletes the earlier one —
// silently, with no visual symptom, undoing the type rewire at runtime. The
// `shadow-color` catch-all does the same to `shadow-elevation-*`. cn() is reconfigured
// (extendTailwindMerge) to file each custom name under the group it ACTUALLY sets:
// `text-<role>` → font-size, `shadow-<name>` → box-shadow. These are the two derived
// name lists that configuration reads, emitted here from the SAME token source the CSS
// is built from, so a role added to base.tokens.json lands in the right merge group with
// no second hand-written list to drift. Nothing else consumes this file; both the
// shipped cn() and scripts/check-type-slots.mjs's Link C import it.
const fontSizeRoles = entries(base)
  .filter(([, token]) => token.$type === "typography")
  .map(([name]) => {
    if (!name.startsWith("type-")) {
      throw new Error(
        `typography composite "${name}" is not named type-*; the text-<role> utility ` +
          `derivation in src/tw-merge-groups.js can't map it. Rename it type-<role>, or ` +
          `update this derivation and system.css's @theme mapping together.`
      )
    }
    return name.slice("type-".length)
  })
const shadowElevationNames = entries(base)
  .filter(([, token]) => token.$type === "shadow")
  .map(([name]) => name)
const ringWidthNames = entries(base)
  .filter(([name, token]) => name.startsWith("ring-") && token.$type === "dimension")
  .map(([name]) => name.slice("ring-".length))

const twMergeGroups = `// GENERATED from tokens/base.tokens.json by scripts/build-tokens.mjs — DO NOT EDIT.
// The custom Tailwind utility names cn() must register with tailwind-merge so its default
// config's catch-all colour / shadow-colour / ring-colour groups stop swallowing them (Issue 07l).
// Derived from the same typography, shadow and ring-width composites the CSS is built from — see
// the emitter's own comment. Not committed; regenerated by the pre* hooks alongside tokens.css.

/** Type-role suffixes: \`text-<suffix>\` sets font-size (+ line-height/weight/tracking). */
export const fontSizeRoles = [
${fontSizeRoles.map((n) => `  "${n}",`).join("\n")}
]

/** Elevation names: \`shadow-<name>\` sets box-shadow, not a shadow colour. */
export const shadowElevationNames = [
${shadowElevationNames.map((n) => `  "${n}",`).join("\n")}
]

/** Ring-width names: \`ring-<name>\` sets ring-width, not a ring colour. */
export const ringWidthNames = [
${ringWidthNames.map((n) => `  "${n}",`).join("\n")}
]
`
writeFileSync(join(root, "src", "tw-merge-groups.js"), twMergeGroups)

console.log(
  `tw-merge-groups: ${fontSizeRoles.length} font-size role(s), ${shadowElevationNames.length} elevation name(s), ${ringWidthNames.length} ring-width name(s) → src/tw-merge-groups.js`
)
