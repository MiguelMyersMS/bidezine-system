...
railSidebarPhases[1] = {
  ...
  subPhases: [
    ...
    { id: "remaining", title: "19 remaining divergence rows still awaiting a decision", status: "pending", note: "See the Full divergence list tab — categories C, F, H, I, J, K, L, M. Category B (dark rail colors) and G (radius) are now fully resolved; 6 rail-specific items (G-1, H-1, K-1, K-2, L-2, M-1) approved once the rail was visually signed off (\u201crail seems right entirely\u201d). D-12 (Panel-actions menu text-size vs. D-5/D-6) was raised and resolved after that sign-off: keep the shared dropdown-menu text-sm convention, no override. L-11 (panel-tree vertical guide line) is a new open item: applied live for preview, awaiting explicit confirmation before being treated as final." },
  ],
}
...
export const divergenceCategories: DivergenceCategory[] = [
  ...
  },
  {
    ...
    rows: [
      ...
      { id: "K-3", what: "SCROLL.css / SCROLL.className (custom scrollbar styling)", status: "decision", detail: "Could use our ScrollArea component, browser-default scrollbars, or a custom stylesheet \u2014 needs a pick. Note: this is panel-content scrolling (the panel's item list uses a plain `overflow-y-auto` div today), not the rail \u2014 the rail itself never scrolls; overflow sections are stashed into the \u201cMore\u201d menu instead." },
