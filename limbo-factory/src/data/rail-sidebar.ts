...
railSidebarPhases[1] = {
  ...
  subPhases: [
    ...
    { id: "remaining", title: "19 remaining divergence rows still awaiting a decision", status: "pending", note: "See the Full divergence list tab — categories C, D, F, H, I, J, K, L, M. Category B (dark rail colors) and G (radius) are now fully resolved; 6 rail-specific items (G-1, H-1, K-1, K-2, L-2, M-1) approved once the rail was visually signed off (\u201crail seems right entirely\u201d). D-12 is a new open item found after that sign-off (Panel-actions menu text-size vs. D-5/D-6)." },
  ],
}
...
export const divergenceCategories: DivergenceCategory[] = [
  ...
  },
  {
    ...
    rows: [
      ...
      { id: "D-12", what: "Panel actions / overflow menu row text renders at the shared primitive's text-sm default, not the already-approved text-xs (D-5/D-6)", status: "decision", detail: "Prompted by \u201cis the font token consistent with our dropdown-menu?\u201d Measured live via getComputedStyle: our rail's DropdownMenuItem/CheckboxItem rows (\u201cExpand all\u201d, \u201cCollapse all\u201d, \u201cSearch box\u201d) render at 14px/400 Inter \u2014 exactly Tailwind's `text-sm`, hard-coded in dropdown-menu.tsx's own className for every item/checkbox/radio/sub-trigger row. Checked origin's real token source (design-system/src/tokens.ts): the equivalent origin rows use TYPE.bodyS (13px/400, unchecked) and TYPE.labelM (13px/500, checked) \u2014 and D-5/D-6 ALREADY approved a bidezine mapping for exactly those two origin tokens: text-xs for \u201ccompact secondary/menu text\u201d (D-5) and text-xs font-medium for \u201cchecked rows\u201d (D-6). So the actually-rendered menu text (text-sm/14px) conflicts with our own already-approved D-5/D-6 decisions (text-xs/12px) for this exact content. Root cause is structural, not a copy mistake: origin's PanelMenuRow is fully custom-styled (Radix is unstyled; origin applies TYPE.bodyS/labelM via inline `style` on every row itself), while our dropdown-menu.tsx is the shadcn-styled wrapper, where text-sm is shadcn's own deliberate, SHARED, system-wide default across every menu-family primitive (Menubar, Select, Context Menu, Command all use it too) \u2014 not something scoped to the rail. Left OPEN, not fixed: unlike M-12/M-16 (unambiguous bugs against confirmed behavior), this is a genuine scope call \u2014 override className to text-xs on just the rail's two menus (honors D-5/D-6 but diverges from bidezine's own shared menu-family convention), or treat D-5/D-6 as not intended to override the shared primitive's own default (leave text-sm, but then D-5/D-6 never actually apply to any real rendered surface). AI does not auto-decide this \u2014 awaiting human input." },
    ],
  },
  {
    id: "E",
